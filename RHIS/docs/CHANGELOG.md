...
## 2026-03-31 — Hotfixes: MiniRHIS troubleshooting

### 2026-03-31 — Hotfixes applied during Satellite post-install troubleshooting

- **Area:** `container/vars/host_vars/satellite_content_profile.yml`, `container/roles/rhis-builder-satellite/roles/lifecycle_environments/tasks/main.yml`, `MiniRHIS.sh`
- **Summary:** Patched recursive templating in `satellite_content_profile.yml`, added missing `label:` fields to lifecycle environment definitions, enabled debug output for the lifecycle_environments tasks and added managed hotfix functions to `MiniRHIS.sh` to apply these fixes into the running provisioner container.
...
- **Summary:**
  ...
  - Expanded `rhis_installer.py` `main()` argparse to include all CLI flags from `MiniRHIS.sh`: `--non-interactive`, `--env-file`, `--inventory`, `--inventory-growth`, `--container-config-only`, `--satellite`, `--idm`, `--aap`, `--attach-consoles`, `--status`, `--reconfigure`, `--test[=fast|full]`, `--demo`/`--DEMO`, `--demokill`/`--DEMOKILL`, `--validate`/`--preflight`, `--generate-env`.
  ...
  - Bulk-replaced all `rhis_install.sh` → `MiniRHIS.sh` and `rhis_install.py` → `rhis_installer.py` references across all `*.md` documentation files.
...
- **Area:** `MiniRHIS.sh`, `rhis_installer.py`, `README.md`, `.PROMPTS_REFERENCE.md`, `CHECKLIST.md`, `host_vars/README.md`, `inventory/README.md`, `docs/README.md`, `QUICK_RECOVERY.md`
- **Summary:**
...
### 2026-03-24 10:50:00 MDT — Merge headless helper files into MiniRHIS.sh

- **Area:** `MiniRHIS.sh` (new functions), `rhis-headless-validate.sh` (removed), `rhis-headless.env.template` (removed)
- **Summary:**
  - Merged standalone `rhis-headless-validate.sh` into `MiniRHIS.sh` as `validate_headless_config()` function.
  - Merged `rhis-headless.env.template` into `MiniRHIS.sh` as `generate_env_template()` function (heredoc).
  ...
  - Removed `rhis-headless-validate.sh` and `rhis-headless.env.template` from repository (functionality now in `MiniRHIS.sh`).
...
- **Reason:** Provide durable, per-run operational logs for troubleshooting and auditability of each `MiniRHIS.sh` execution.
